...
def main():
    ...
    parser_topics.add_argument("--print-log-format", help="Print log format (e.g., json)", default="text")
    parser_topics.add_argument("--print-log-file", help="File to write JSON log output")

    # news-list subcommand  
    parser_list = subparsers.add_parser("news-list", help="Crawl news list for a topic")
    parser_list.add_argument("--input-file", required=True, help="Input topic list JSON file")
    parser_list.add_argument("--topic-index", type=int, required=True, help="Topic index (0-based)")
    parser_list.add_argument("--output-file", help="Output file path for news list JSON")
    parser_list.add_argument("--print-log-format", help="Print log format (e.g., json)", default="text")
    parser_list.add_argument("--print-log-file", help="File to write JSON log output")

    # news-details subcommand
    parser_details = subparsers.add_parser("news-details", help="Crawl news details")
    parser_details.add_argument("--input-file", required=True, help="Input news list JSON file")
    parser_details.add_argument("--output-folder", help="Output folder for news detail JSON files")
    parser_details.add_argument("--print-log-format", help="Print log format (e.g., json)", default="text")
    parser_details.add_argument("--print-log-file", help="File to write JSON log output")

    args = parser.parse_args()

    if args.command == "news-topics":
        start_time = time.time()
        output_manager = OutputManager(args.print_log_format, args.print_log_file)
        
        topics = crawl_news_topics(args.output_file, output_manager)
        
        if args.output_file:
            os.makedirs(os.path.dirname(args.output_file), exist_ok=True)
            with open(args.output_file, "w", encoding="utf-8") as f:
                json.dump(topics, f, ensure_ascii=False, indent=2)
            output_manager.info(f"Saved topics to {args.output_file}")

        elapsed_time = time.time() - start_time
        
        if args.print_log_format == 'json':
            output_data = {
                "timestamp": datetime.now().isoformat(),
        ...
        news_data = crawl_news_list(args.input_file, args.topic_index, args.output_file, output_manager)
        
        if news_data and args.output_file:
            os.makedirs(os.path.dirname(args.output_file), exist_ok=True)
            with open(args.output_file, "w", encoding="utf-8") as f:
                json.dump(news_data, f, ensure_ascii=False, indent=2)
            output_manager.info(f"Saved news list to {args.output_file}")
        ...
        if args.print_log_format == 'json':
            output_data = {
                "timestamp": datetime.now().isoformat(),
                "elapsed-time": f"{elapsed_time:.2f}s",
                "topic-index": args.topic_index,
                "total-news-list": news_data["total_news"] if news_data else 0,
                "output-file": args.output_file
            }
            output_manager.json_output(output_data)
        elif not args.output_file and news_data:
            print(json.dumps(news_data, ensure_ascii=False, indent=2))

    elif args.command == "news-details":
        start_time = time.time()
        output_manager = OutputManager(args.print_log_format, args.print_log_file)
        
        result = crawl_news_details(args.input_file, args.output_folder, output_manager)
