...
export interface CrawlTopicsOptions {
  ...
  testIndex: number;  // Test mode: use mock worker URL with index (1, 2, 3, etc.)
  newscastID?: string;  // Reuse existing newscast ID (if not provided, generates new one)
}
...
export async function crawlNewsTopics(options: CrawlTopicsOptions): Promise<CrawlTopicsResult> {
  // Use mock URL with testIndex
  const url = `https://ai-newscast-bigkinds-mock-worker.r-s-account.workers.dev/${options.testIndex}`;
  ...
  console.log(`[CRAWL_TOPICS OPTIONS] includeHTML: ${options.includeHTML}, offset: ${offset}, limit: ${limit ?? 'all remaining'}, existingTopics: ${options.existingTopics ? 'provided' : 'none'}, testIndex: ${options.testIndex}`);
  ...
  try {
    ...
    // Check for duplicates if existingTopics is provided (BEFORE offset/limit)
    let duplicates: Array<{ title: string; issueName: string }> | undefined;
    let deduplicatedTopics = topics;
    if (options.existingTopics) {
      ...
      const duplicateTopics = topics.filter(t =>
      ...
      if (duplicateTopics.length > 0) {
        ...
        const duplicateTitles = new Set(duplicateTopics.map(t => t.title));
        deduplicatedTopics = topics.filter(t => !duplicateTitles.has(t.title));
        console.log(`[CRAWL_TOPICS DUPLICATES] Removed duplicates, ${deduplicatedTopics.length} topics remaining`);
      } else {
    ...
    const slicedTopics = limit !== undefined
      ? deduplicatedTopics.slice(offset, offset + limit)
      : deduplicatedTopics.slice(offset);  // Take all topics from offset to end
    ...
    // Merge with existing topics if provided
    let allTopics = validatedTopics;
    if (options.existingTopics) {
      console.log(`[CRAWL_TOPICS MERGE] Merging ${options.existingTopics.count} existing topics with ${validatedTopics.length} new topics`);
      allTopics = [...options.existingTopics.topics, ...validatedTopics];
      console.log(`[CRAWL_TOPICS MERGE] Total topics after merge: ${allTopics.length}`);
    }
    ...
    // Calculate content metrics (using ALL topics for final result)
    const newsCountsPerTopic = allTopics.map(t => t.news_ids.length);
    const totalNewsIDs = newsCountsPerTopic.reduce((sum, count) => sum + count, 0);
    const averageNewsPerTopic = totalNewsIDs / allTopics.length;
    ...
    const jsonBytes = JSON.stringify(allTopics).length;
    const totalBytes = htmlBytes + jsonBytes;

    // Use provided newscast ID or generate new one from timestamp
    const newscastID = options.newscastID ?? startedAt.replace(/[:.]/g, '-');
    const date = newscastID.split('T')[0];

    // Return result based on options (with ALL topics merged)
