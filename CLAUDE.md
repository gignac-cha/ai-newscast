...
- **실시간 자막 시스템 완성**: 오디오 재생에 맞춘 스크립트 자막 표시 시스템 구현
  - **시간 기반 스크립트 매칭**: `audio-files.json`의 `duration_seconds` 정보로 정확한 자막 타이밍
  - **CurrentScriptDisplay 컴포넌트**: 하단 바 위에 고정된 투명 자막 영역
  - **useCurrentScript 훅**: 누적 시간 계산으로 현재 재생 구간의 스크립트 자동 감지
  - **호스트 이름 표시**: `host1`, `host2` → 실제 호스트 이름 변환 (예: "김다연", "이준호")
  - **뱃지 스타일 자막**: 호스트 이름을 cyan 색상 뱃지로 표시, 스크립트와 인라인 배치
- **소스 링크 시스템 완성**: 클릭 가능한 뉴스 소스 링크 구현
  - **Popover 기반 소스 목록**: 각 소스 뱃지 클릭 시 해당 소스의 전체 기사 목록 표시
  - **실제 링크 연결**: 기사 제목이 원본 URL로 연결되어 새 탭에서 열림
  - **시각적 개선**: 카드 스타일 링크 + 외부 링크 아이콘 + 호버 효과
  - **소스 정렬**: 기사 수가 많은 순서로 내림차순 정렬
- **오디오 상태 동기화 완성**: AudioContext 도입으로 컴포넌트 간 상태 동기화 해결
  - **전역 오디오 상태**: 모든 컴포넌트가 동일한 오디오 상태 공유
  - **토픽 전환 최적화**: 토픽 변경 시 이전 오디오 완전 정리 및 스피너 문제 해결
  - **클릭 영역 확장**: 소스/시간 뱃지 클릭으로도 카드 펼치기/접기 가능
- **UI/UX 개선**:
  - **메타데이터 배치 최적화**: 접힌 상태에서 sources + 시간 뱃지를 `justify-content: space-between`으로 양 끝 배치
  - **중복 제거**: 펼친/접힌 상태에 따른 조건부 메타데이터 표시로 중복 방지
  - **투명도 조절**: 자막 박스 배경을 `rgba(0, 0, 0, 0.6)`으로 적절한 투명도 확보
